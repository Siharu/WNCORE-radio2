...
(function () {
  ...
  function patchSwipeHint() {
    // [NEUTRALIZED: swipe hint wrapper — handled by unified hook in bundle.js]
  }
...
(function WNCORE_BUGFIX2() {
  ...
  // [NEUTRALIZED: fixPlayStationAbortCascade — AbortError guard + debounce 
  //  handled by unified hook in bundle.js which provides _playLocked + 400ms debounce]
...
/* ═══════════════════════════════════════════════════════════════════════
   WNCORE RADIO — UNIFIED PLAYSTATION HOOK
   Installed ONCE after all code is loaded. Replaces the 8-wrapper chain
   with a single function that does everything in-order, no race conditions.
   ═══════════════════════════════════════════════════════════════════════ */
(function installUnifiedPlayStation() {
  'use strict';

  // Wait until the DOM + main.js have defined the base playStation
  function waitAndInstall(attempts) {
    if (attempts > 80) {
      console.error('[WNCORE] Unified hook: playStation never defined after 8s — giving up');
      return;
    }
    if (typeof window.playStation !== 'function') {
      setTimeout(function() { waitAndInstall((attempts||0) + 1); }, 100);
      return;
    }
    // If already installed by a previous run (hot-reload), skip
    if (window.playStation._unifiedHook) return;

    var _basePlayStation = window.playStation;

    // ── Debounce state (shared, single instance) ───────────────────────
    var _lastPlayTs = 0;
    var _playLocked = false;
    var _playRetryTimer = null;
    var _sessionTimer = null;

    window.playStation = function unifiedPlayStation(url, name, meta, emoji) {
      var now = Date.now();

      // ── GUARD 1: Rapid-fire debounce (400ms) ─────────────────────────
      if (now - _lastPlayTs < 400) return;
      _lastPlayTs = now;

      // ── GUARD 2: AbortError cascade lock ─────────────────────────────
      if (_playRetryTimer) { clearTimeout(_playRetryTimer); _playRetryTimer = null; }
      if (_playLocked) {
      ...
      // ── Call the base (main.js) playStation ───────────────────────────
      try {
        _basePlayStation(url, name, meta, emoji);
      } catch(e) {
        _playLocked = false;
        console.warn('[WNCORE] playStation base threw:', e);
        return;
      }
      ...
      setTimeout(function() { if (typeof updateMbnDot === 'function') updateMbnDot(); }, 1000);

      // From improvements.js patchPlayStationV2
      ...
      var _audioEl = document.getElementById('audio');
      if (typeof startWaveformDraw === 'function' && _audioEl) { try { startWaveformDraw(_audioEl); } catch(e) {} }
      try { document.dispatchEvent(new CustomEvent('wncore-station-changed', { detail: { name: name, url: url } })); } catch(e) {}

      // From mini-player wrapper (improvements.js section 3)
      try {
        var _miniNameEl = document.getElementById('mini-name');
        if (_miniNameEl) _miniNameEl.textContent = name || '— receiving —';
        if (typeof window.__miniTogglePlay !== 'undefined') {
          var _pI = document.getElementById('mini-play-icon');
          var _paI = document.getElementById('mini-pause-icon');
          if (_pI) _pI.style.display = 'none';
          if (_paI) _paI.style.display = '';
          var _w = document.getElementById('mini-wave');
          if (_w) _w.style.opacity = '1';
        }
      } catch(e) {}

      // From wncore-upgrades.js session tracker
      ...
      setTimeout(function() {
        document.querySelectorAll('body > div').forEach(function(el) {
          if (el.style && el.style.position === 'fixed' && el.style.bottom === '90px' &&
              el.textContent && el.textContent.indexOf('Swipe') !== -1) {
            var playerH = parseInt(
              getComputedStyle(document.documentElement).getPropertyValue('--player-h') || '76'
            );
    ...
    // Mark with ALL flags so no remaining wrapper code re-wraps this
    window.playStation._unifiedHook = true;
    ...
    window.playStation._wnPatched = true;
    window.playStation._p5Patched = true;
    window.playStation._nullSafe = true;
    window.playStation._swipeFixed = true;
    window.playStation._cardFixed = true;
    window.playStation._exitFixed = true;
    window.playStation._mbnSynced = true;
    window.playStation._miniPatched = true;

    console.log('%c[WNCORE] Unified playStation hook installed — all wrappers consolidated', 
      'color:#c8472a;font-family:monospace;font-size:11px');
  }

  // Run after DOM ready
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() { waitAndInstall(0); });
  } else {
    waitAndInstall(0);
  }
})();
